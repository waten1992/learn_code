...
out:
	/* set init read_pos 1 if it's 0. avoid just exist reader read ringbuf */
	if (0 == dis->read_pos[i])
	{
...
disruptor_next_write_pos(struct disruptor* dis)
{
	...
	do 
	{
		...
		/* update the slowest_read_pos */
		__atomic_store_n(&dis->slowest_read_pos, slowest_read_pos, __ATOMIC_RELAXED);
		/* ensure writer can't write slowest_read_pos  */
...
disruptor_write_done(struct disruptor* dis, unsigned long pos)
{
	...
	/* ensure max_readable_pos equal the next write  */
	while (__atomic_load_n(&dis->max_readable_pos, __ATOMIC_RELAXED) != max_readable_pos)
	{
		usleep(1);
	}
	/* update max_readable_pos after write */
